...
hidden_layer = keras.layers.LeakyReLU(negative_slope=0.1)(keras.layers.Dense(2024)(hidden_layer))
...
actor_hidden_layer = keras.layers.Dense(2024, activation="tanh")(hidden_layer)
actor = keras.layers.Dense(env.n*env.n*n_moves, activation='softmax')(actor_hidden_layer)

critic_hidden_layer = keras.layers.LeakyReLU(negative_slope=0.05)(keras.layers.Dense(2024)(hidden_layer))
...
for episode in range(num_episodes):
    ...
    with GradientTape() as tape:
        ...
        for batch_i in range(batch_size):
            env.reset()
            rewards = []
            for t in range(1): #range(2+int(np.log2(1+episode))):  # Limit the number of time steps
