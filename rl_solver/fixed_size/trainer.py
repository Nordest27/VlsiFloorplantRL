...
embedding_input_layer = keras.layers.Input((env.n + env.n,))
embeddings_layer = keras.layers.Embedding(env.n, 4)(embedding_input_layer)
embeddings_layer = keras.layers.Flatten()(embeddings_layer)

reals_input_layer = keras.layers.Input((env.n*4 + env.n*env.n,))

hidden_layer = keras.layers.Concatenate()([embeddings_layer, reals_input_layer])
hidden_layer = keras.layers.Dense(2048, activation="tanh")(hidden_layer)
hidden_layer = keras.layers.LeakyReLU(negative_slope=0.05)(keras.layers.Dense(2048)(hidden_layer))
# hidden_layer = keras.layers.LeakyReLU(negative_slope=0.05)(keras.layers.Dense(2048)(hidden_layer))
...
model = keras.Model(inputs={"xy": embedding_input_layer, "weights": reals_input_layer}, outputs=[wfa, wsa, wma, critic])
"""
num_hidden = 128
...
for episode in range(num_episodes):
    ...
    with GradientTape() as tape:
        ...
        for t in range(1, 100):# 2+int(np.log(1+episode))):  # Limit the number of time steps
            ...
            inp = keras.ops.convert_to_tensor(env.flattened_observation())
            wfp, wsp, wmp, critic_value = model({
                "xy": keras.ops.expand_dims(
                    inp[:2*env.n], 0
                ),
